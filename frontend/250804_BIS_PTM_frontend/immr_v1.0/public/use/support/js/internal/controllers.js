  ...
  function ($scope, $log, $http) {
    ...
    $scope.prepareDataToProcess = function (topSpectrum = true) {
      ...
      submitData = topSpectrum
        ? $scope.db.items.map(({ mZ, intensity }) => ({ mZ, intensity }))
    ...
    */
    ...
    $scope.updateSpectrum = function (data) {
      $scope.db.items = data.mz.map((mz, index) => ({
        mZ: mz,
        intensity: data.intensity[index],
      }));
      $scope.dbBottom.items = data.mz.map((mz, index) => ({
        mZ: mz,
        intensity: data.intensity[index],
      }));

      $scope.peptide.precursorCharge = data.charge;
      $scope.peptide.sequence = data.sequence;
